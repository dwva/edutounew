...
const Navbar = () => { 
  ...
  const collapsedHeight = 40; // Height when collapsed
  const expandedHeight = 40 + 20 + (navItems.length * 51); // Header + padding + (items * item height with margins)
      ...
      <nav className={`fixed top-10 left-0 w-full px-4 md:px-4 z-50 flex items-start justify-between ${isMobile ? 'py-2' : 'py-3'}`}> 
        ...
        {!isMobile ? ( 
          <LogoContainer 
            ...
            $isMobile={isMobile} 
          > 
            <LogoHeader onClick={toggleExpanded} expanded={expanded}> 
              <LogoImage src={logo} alt="Edutou Logo" /> 
              <motion.div 
                animate={{ rotate: expanded ? 220 : 0 }} 
            ...
            > 
              {navItems.map((item) => ( 
                <MobileNavItem key={item.name}> 
                  <MobileNavLink to={item.path} onClick={toggleMobileMenu}> 
                    {item.name} 
                  </MobileNavLink> 
                </MobileNavItem> 
              ))} 
